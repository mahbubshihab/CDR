    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-5 gap-4">
        {[
          ...
          { label: 'ACTIVITY %', value: `${globalStats.activityPercentage}%`, color: 'text-blue-400' },
          { label: 'MISSING %', value: `${globalStats.missingPercentage}%`, color: 'text-yellow-500' }
        ].map((item, i) => (
          <div key={i} className="bg-[#131f37] border border-[#1e293b] rounded-lg p-5 flex flex-col items-center justify-center text-center shadow-sm h-28">
      ...
      <div className="grid grid-cols-2 gap-4 h-80">
        ...
        <div className="bg-[#131f37] border border-[#1e293b] rounded-lg p-4 flex flex-col">
      ...
      <div className="bg-[#131f37] border border-[#1e293b] rounded-lg p-4">
        <h3 className="text-sm font-bold text-white mb-4">Monthly statistics</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-gray-400 font-medium uppercase border-b border-[#1e293b]">
              <tr>
            ...
            <tbody className="divide-y divide-[#1e293b]">
              {monthlyStats.map((stat, idx) => (
                <tr key={idx} className="hover:bg-[#1e293b]/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-center text-blue-400">{stat.total > 0 ? ((stat.active / stat.total) * 100).toFixed(2) : '0'}%</td>
      ...
      <div className="grid grid-cols-4 gap-4 mt-2">
        {[
          ...
          <div key={i} className="bg-[#131f37] border border-[#1e293b] rounded-lg p-5 flex flex-col items-center justify-center text-center shadow-sm h-24">
