...
/**
 * Floating WhatsApp button in the bottom right corner of the screen
 */
export function FloatingWhatsAppWidget() {
  return (
    <div className="fixed bottom-5 right-5 z-40 flex items-center no-print">
      <a
        href={WHATSAPP_URL}
        target="_blank"
        rel="noopener noreferrer"
        className="group flex items-center gap-2.5 px-3.5 py-2.5 bg-[#25D366] hover:bg-[#20ba57] text-white rounded-full shadow-[0_4px_20px_rgba(37,211,102,0.4)] hover:shadow-[0_6px_25px_rgba(37,211,102,0.6)] transition-all transform hover:scale-105 cursor-pointer"
        title={`Direct WhatsApp Support: ${WHATSAPP_NUMBER}`}
      >
        <span className="relative flex h-3 w-3">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
          <span className="relative inline-flex rounded-full h-3 w-3 bg-white"></span>
        </span>
        <WhatsAppIcon className="h-4.5 w-4.5 fill-current shrink-0" />
        <span className="text-xs font-bold font-sans hidden sm:inline-block tracking-wide">
          WhatsApp Support
        </span>
      </a>
    </div>
...
/**
 * Sidebar promotional section
 */
export function SidebarPromoBox({ collapsed }: { collapsed?: boolean }) {
  if (collapsed) {
    return (
      <div className="p-2 border-t border-[#2e2e2e] bg-[#141414]/80 flex flex-col items-center gap-2 shrink-0">
        <a
          href={LAC_ANALYZER_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="p-2 text-[#3ecf8e] hover:text-white hover:bg-[#3ecf8e]/15 border border-[#3ecf8e]/30 rounded-xl transition-all cursor-pointer flex items-center justify-center"
          title="LAC Cell Analyzer (laccelldecoder.com)"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="p-2 text-[#25D366] hover:text-white hover:bg-[#25D366]/15 border border-[#25D366]/30 rounded-xl transition-all cursor-pointer flex items-center justify-center"
  ...
  return (
    <div className="p-3 border-t border-[#2e2e2e] bg-[#141414]/80 space-y-2 shrink-0">
      <div className="flex items-center justify-between px-1">
        <span className="text-[10px] font-bold uppercase tracking-wider text-gray-500 font-mono">
          Featured & Support
        </span>
        <span className="h-1.5 w-1.5 rounded-full bg-[#3ecf8e] animate-pulse"></span>
      </div>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center justify-between p-2.5 rounded-xl bg-gradient-to-r from-emerald-500/10 via-[#3ecf8e]/15 to-teal-500/10 border border-[#3ecf8e]/30 hover:border-[#3ecf8e] text-[#3ecf8e] hover:text-white transition-all text-xs font-semibold group cursor-pointer shadow-[0_0_12px_rgba(62,207,142,0.1)] hover:shadow-[0_0_18px_rgba(62,207,142,0.25)]"
      >
        <div className="flex items-center gap-2">
          <div className="h-6 w-6 rounded-lg bg-[#3ecf8e]/15 flex items-center justify-center">
            <Radio className="h-3.5 w-3.5 text-[#3ecf8e]" />
          </div>
          <div className="text-left">
            <span className="block leading-tight font-bold">LAC Cell Analyzer</span>
            <span className="text-[9px] text-gray-400 font-normal font-mono">laccelldecoder.com</span>
          </div>
        </div>
        <ExternalLink className="h-3.5 w-3.5 text-[#3ecf8e]/70 group-hover:text-white group-hover:translate-x-0.5 transition-transform" />
      </a>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center justify-between p-2.5 rounded-xl bg-[#25D366]/10 border border-[#25D366]/30 hover:border-[#25D366] hover:bg-[#25D366]/20 text-[#25D366] hover:text-white transition-all text-xs font-semibold group cursor-pointer shadow-[0_0_12px_rgba(37,211,102,0.1)] hover:shadow-[0_0_18px_rgba(37,211,102,0.25)]"
      >
        <div className="flex items-center gap-2">
          <div className="h-6 w-6 rounded-lg bg-[#25D366]/15 flex items-center justify-center">
            <WhatsAppIcon className="h-3.5 w-3.5 fill-current" />
          </div>
          <div className="text-left">
            <span className="block leading-tight font-bold">WhatsApp Support</span>
            <span className="text-[9px] text-gray-400 font-normal font-mono">{WHATSAPP_NUMBER}</span>
          </div>
        </div>
        <ExternalLink className="h-3.5 w-3.5 text-[#25D366]/70 group-hover:text-white group-hover:translate-x-0.5 transition-transform" />
      </a>
    </div>
  );
}
